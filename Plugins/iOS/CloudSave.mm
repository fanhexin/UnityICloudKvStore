...
@implementation CloudSave

static char* const CallbackObject = (char*)"iCloudKvStoreListener";
static char* const CallbackMethod = (char*)"ExternalChange";

-(id)init
{
    NSLog(@"CloudSave init");
    self = [super init];
...
- (const char*) getDevString: (NSString*) key
{
    if ([NSUbiquitousKeyValueStore defaultStore])
    {
        ...
        //return [[iCloudKVStore stringForKey:key] UTF8String];
...
static CloudSave* cloudSaveObject = nil;

void EnsureCloudSaveObjectIsNotNil()
{
    if (cloudSaveObject == nil) {
        cloudSaveObject = [[CloudSave alloc] init];
    }
}
